...
def _apply_env_aliases() -> None:
    """Map Railway / OpenAlice-style env names onto this project's variables."""
    aliases = (
    ...
    if paper is not None and not os.getenv("ALPACA_USE_PAPER"):
        p = paper.strip().lower()
        os.environ["ALPACA_USE_PAPER"] = "False" if p in ("false", "0", "live") else "True"
    ...
    if gemini_model:
        if not os.getenv("QUICK_THINK_LLM"):
            os.environ["QUICK_THINK_LLM"] = gemini_model
        if not os.getenv("DEEP_THINK_LLM"):
            os.environ["DEEP_THINK_LLM"] = gemini_model
